...
done_file=${done_file_path}/action_noauth.done
source ${done_file}

incre_click=noauth_incre_click_${file_time_flag}
new_end_time_stamp=${end_timestamp}
last_end_time_stamp=${end_timestamp}
:>${incre_click}

# ~/disk/Hive/data/clicked_shitu_log
for file in `ls  /home/work/disk/Hive/data/clicked_shitu_log/click_shitu_noauth_[0-9]* | tac`;do
    timeflag=${file:0-10}
...
python process_install_num_noauth.py ${incre_click} ${incre_click}.tag
if [[ $? -ne 0 ]];then
