...
#            print raw_line;
        ...
        if line[49] == "1.000000":
            isnew = 1
        try:
            fp_w.write("%s\001%s\001%s\001%s\001%s\n"%(raw_line.rstrip("\r\n"),"insnum"+str(installnum),str(isnew),meta_info["offer_country_click"],meta_info["offer_country_conversion"]));
        except:
            fp_w.write("%s\001%s\001%s\001%s\001%s\n"%(raw_line.rstrip("\r\n"),"insnum"+str(installnum),str(isnew),0,0));
    fp_w.close()
